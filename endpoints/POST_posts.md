...
#### Add a new Native Post

Native Posts can be posted to the Wall right away, or scheduled to be posted later. 
You can add an image or video to your post by specifying a publicly available URL, or by uploading it via [**POST** `/media_upload`][POST /media_upload].

It is allowed to omit the `text` parameter if an `image` or `video` is added, and vice versa. 
It is not allowed to omit all three of those fields at the same time. Same goes for `user_name` and `user_image`.

If you add a `video` **and** an `image` then the latter will be uses as a preview image for the video. 
If only a `video` is set then a preview image will be automatically generated.
...
- `video` *(required if `text` and `image` are omitted)*: Video of the post.
  ...
  - This can either be a full URL to a video, or an ID returned by [**POST** `/media_upload`][POST /media_upload]
- `image` *(required if `text` and `video` are omitted)*: Main image of the post. If `video` is also set then this image is only used as a preview for the video.
  ...
  - This can either be a full URL to an image, or an ID returned by [**POST** `/media_upload`][POST /media_upload]
...
- `link`: The URL that a click on the post's timestamp, or the image in the post detail view leads to.
- `language` The post's language. If no value is given, we try to detect the language from the `text` field. [Languages]
...
[POST /media_upload]: POST_media_upload.md "Upload videos via our API"
